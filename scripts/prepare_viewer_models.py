    ...
    with tempfile.TemporaryDirectory(dir=destination.parent, prefix='.models-') as temp:
        ...
        for name, (label, labels) in NAMES.items():
            ...
            if name == 'tech-core':
                ...
                note = 'CAD display rig with six movable axes and schematic bearings. The reference export keeps these axes as fixed frames.'
            ...
            for joint, joint_label in zip(binding['joints'], labels, strict=True):
                ...
                joints.append({'id': joint['id'], 'label': joint_label, 'type': joint['type'], 'motion_node': joint['motion_node'], 'axis': joint['axis'], 'limits': limits})
