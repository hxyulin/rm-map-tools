...
See the [performance and RAM comparison](performance.md) for the whole-file
M5 Max and SolidWorks baselines, machine attribution, and measurement scope.
...
| whole-file OCCT conversion on M5 Max, including GLB write | 9 h, ~40 GB | not attempted |
...
- `inspect` reproduces the counts of the original handoff notes on all three files.
...
onto V2.0.0 geometry) and `report`.
