...
Date: 2026-09-10. Machine: this Mac (M3 Pro, 18 GiB). Tools: `prototype/*.py`
(Python 3.12 via `uv`, numpy, `cadquery-ocp` 8.0.1 for OCCT).

Result: **the conversion is proven.** Both 1 GB files split into standalone
...
`inspect` numbers reproduced exactly from HANDOFF §2: V2.0.0 429 products,
...
## 3. Facts learned about the files (add to HANDOFF §2/§5)
...
- V2.0.0 equipment is flat: `26062900` (674 MB, 67 bodies), `0003`
  ...
  no sub-assemblies. A per-body split is needed for these (HANDOFF §10).
...
`prototype/mesh_parts.py <pkg> <index.npz> out.bin out.json [--lin 4 --ang 0.5
...
The page builder and template live in `prototype/preview/`; see `previews.md`.
...
- Part files (2.2 GB, regenerable in 4 minutes) stay in the session
  ...
  `coverage.json`) are copied to `out/{v20,v12,rmul}/` in this repo.
- Regenerate: `python3 prototype/p21index.py <stp> <out.npz>`,
  `python3 prototype/p21split.py <npz> <pkg_dir>` (source path is stored in the npz),
  `ocpenv/bin/python prototype/validate_parts.py <pkg_dir> [--mesh]`,
  `python3 prototype/compare_glb.py <pkg_dir> <glb>`,
  `python3 prototype/coverage.py <npz> <coverage.json>`.
...
`crates/step21` + `crates/rm-map-tools` port the three prototype stages.
