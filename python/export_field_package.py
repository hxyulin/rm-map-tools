...
from mesh_parts import occurrence_transforms, tessellate, tessellate_pair  # noqa: E402
...
COLLISION_CONTRACT = (
    "Independent source tessellations, with explicit collision exclusions recorded "
    "in export_policy; no hull, voxel or legacy proxy substitution. "
    "Sheet surfaces require two-sided contacts. See export_policy for tolerances."
)
...
class GlbWriter:
    ...
    def __init__(self, root_name, asset):
...
def main():
    ...
    generator = provenance.generator("export_field_package.py")
    asset = provenance.glb_asset("export_field_package.py", source_path, source_hash, frame=ARENA_FRAME)
    visual = {"floor": GlbWriter("FLOOR", asset), "arena-static": GlbWriter("ARENA-STATIC", asset)}
    collision = {
        "floor": GlbWriter("FLOOR_COLLISION", asset),
        "arena-static": GlbWriter("ARENA-STATIC_COLLISION", asset),
    }
    ...
    for i, p in enumerate(arena):
        ...
        if err:
            ...
            continue
        settings = a.export_policy.settings(asset, p["name"])
        mesh_stats = {}
        fine, coarse = tessellate_pair(doc, settings, stats=mesh_stats)
        del doc, rd
    ...
    for spec in a.graft:
        ...
        for gname in names.split(","):
            ...
            settings = a.export_policy.settings("undulating-road", gname)
            mesh_stats = {}
            fine, coarse = (None, None) if err else tessellate_pair(doc, settings, stats=mesh_stats)
            if fine is None or coarse is None:
    ...
    for asset in ("floor", "arena-static"):
        assets[asset] = {
            ...
            "tessellation": a.export_policy.resolved.get(asset, {}),
