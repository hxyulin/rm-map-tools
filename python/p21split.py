#!/usr/bin/env python3
"""Text-level per-product split of a STEP Part 21 file (Python counterpart of the Rust `split`).
...
class Splitter:
    ...
    def closure(self, p, solids_only=False):
        ...
        return self.closure_products([p], solids_only)

    def closure_products(self, products, solids_only=False, body_ids=None):
        """Closure of several products at once (see `closure`): the reverse
        boilerplate is filtered against the union, so the result is one
        consistent file rather than a merge of per-product files. Optional body_ids
        selects explicit source bodies, including sheet models, without changing
        their geometry, styles or product placement."""
        ix = self.ix
        seeds, reps = [], set()
        for p in products:
            s, r = self.product_seeds(p)
            seeds += s; reps |= r
        mask = np.zeros(ix.n, dtype=bool)
        rewritten = {}
        blocked = np.array([b for b, rep in self.m.bodies if rep in reps
                            and ((solids_only and self.ix.tname(b) not in SOLID_TYPES)
                                 or (body_ids is not None and int(ix.ids[b]) not in body_ids))], dtype=np.int64)
        if len(blocked):
    ...
    def write(self, p, path, solids_only=False):
        ...
        return self.write_closure(mask, rewritten, path)

    def write_closure(self, mask, rewritten, path, extra=(), header=None):
        """Write the masked rows (rewritten ones replaced) as a STEP file;
        `extra` entity texts are appended before ENDSEC, `header` replaces
        the source header. Returns (rows, rewritten, bytes)."""
        ix = self.ix
        ...
        with open(path, 'wb') as f:
            f.write(header if header is not None else ix.header); f.write(self.crlf)
            ...
            for t in extra:
                f.write(t); f.write(self.crlf); nbytes += len(t)
