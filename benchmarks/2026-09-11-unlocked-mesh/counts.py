...
report={}
packages={'before':root.parent/'rm-simulator/local-assets/field','after':root/'out/unlocked-default'}
for name,path in packages.items():
    ...
    result=subprocess.run([str(root.parent/'rm-simulator-unlocked/target/debug/examples/inspect_assets'),str(path)],capture_output=True,text=True,check=True)
