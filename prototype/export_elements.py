...
Assembly products that carry thousands of single-face sheet bodies next to
their solids are split solids-only (`p21split.Splitter(..., solids_only)`)
into `<pkg>/solids/` before reading, so no whole 300 MB part has to go
through OCCT.
...
Usage:
  export_elements.py <pkg_dir> <index.npz> --rules rules/elements-v1.2.0.json
      ...
      [--collision-lin 10] [--collision-ang 0.7]
...
from mesh_parts import nauo_transforms, occurrence_transforms, tessellate  # noqa: E402
from p21index import Index  # noqa: E402
from p21model import Model  # noqa: E402
from p21split import SOLID_TYPES, Splitter, part_filename  # noqa: E402
from validate_parts import read_part  # noqa: E402

GENERATOR = "rm-map-tools export_elements.py"
LOCAL_FRAME = (
...
def read_glb_nodes(path):
    """Nodes of a glb written by GlbWriter: (name, positions_m, triangles, colour keys)."""
    data = open(path, "rb").read()
    _, _, total = struct.unpack_from("<III", data, 0)
    off, chunks = 12, {}
    while off < total:
        ln, ty = struct.unpack_from("<II", data, off)
        chunks[ty] = data[off + 8 : off + 8 + ln]
        off += 8 + ln
    js = json.loads(chunks[0x4E4F534A])
    bin_ = chunks[0x004E4942]

    def accessor(i):
        a = js["accessors"][i]
        v = js["bufferViews"][a["bufferView"]]
        comp = {5126: np.float32, 5125: np.uint32}[a["componentType"]]
        n = {"VEC3": 3, "SCALAR": 1}[a["type"]]
        arr = np.frombuffer(bin_, comp, count=a["count"] * n, offset=v["byteOffset"] + a.get("byteOffset", 0))
    ...
    for node in js["nodes"]:
        ...
        for prim in js["meshes"][node["mesh"]]["primitives"]:
            ...
            key = js["materials"][prim["material"]]["name"][len("colour_") :]
            P.append(p)
            T.append(idx + nv)
            K += [None if key == "default" else key] * len(idx)
            nv += len(p)
        yield node["name"], np.vstack(P), np.vstack(T), K
...
class Package:
    ...
    def part(self, row):
        return self.by_id.get(int(self.ix.ids[row]))

    def solids(self, row):
        p = self.part(row)
        return [b for b in p["bodies"] if b["type"] in SOLID_TYPES] if p else []

    def solids_file(self, row):
        """Part file holding only the product's solids: the split part when it
        has no sheet bodies, otherwise a cached solids-only split."""
        p = self.part(row)
        if len(self.solids(row)) == len(p["bodies"]):
            return os.path.join(self.pkg, p["file"]), False
        path = os.path.join(self.pkg, "solids", part_filename(p["name"], p["product_id"]))
        if not os.path.exists(path):
            os.makedirs(os.path.dirname(path), exist_ok=True)
            if self.splitter is None:
                self.splitter = Splitter(self.ix, self.m)
            self.splitter.write(row, path, solids_only=True)
        return path, True
    ...
    def placed_box(self, items):
        ...
        pts = [bbox_corners(b, M) for row, M in items for b in self.solids(row)]
        if not pts:
            return None
        P = np.vstack(pts)
...
def mesh_item(pkg, row, lin, ang, clin, cang):
    path, solids_only = pkg.solids_file(row)
    doc, rd, err = read_part(path)
    if err:
        return None, None, err, path
    coarse = tessellate(doc, clin, cang)
    fine = tessellate(doc, lin, ang)
...
def mesh_cluster(pkg, items, a, element, failures, bbox_errors):
    ...
    out = []
    for row, M in items:
        solids = pkg.solids(row)
        if not solids:
            continue
        p = pkg.part(row)
        fine, coarse, err, path = mesh_item(pkg, row, a.lin, a.ang, a.collision_lin, a.collision_ang)
        if err:
...
def main():
    ...
    full = {"visual": GlbWriter("FULL-MAP", GENERATOR), "collision": GlbWriter("FULL-MAP_COLLISION", GENERATOR)}
    composed = []
    ...
    for name in ("floor", "arena-static"):
        ...
        for kind in ("visual", "collision"):
            shutil.copyfile(os.path.join(a.field, entry[kind]), os.path.join(a.out, entry[kind]))
            if sha256(os.path.join(a.out, entry[kind])) != entry[f"{kind}_sha256"]:
                sys.exit(f"{entry[kind]} does not match the field manifest checksum")
    ...
    for spec in rules["elements"]:
        ...
        t1 = time.time()
        visual = GlbWriter(name.upper(), GENERATOR)
        collision = GlbWriter(f"{name.upper()}_COLLISION", GENERATOR)
        ...
        if "graft" in spec:
            ...
            boxes_ref = [tuple(np.asarray(b) - origin for b in pkg.placed_box([item])) for item in clusters[ref] if pkg.solids(item[0])]
                ...
                if anchors is not None:
                    ...
                    for row, M in clusters[k]:
                        if row not in at_ref:
                            unmatched.append(pkg.m.pname(row))
                            continue
                        D = A @ at_ref[row] - in_metres(M)
                        d_rot, d_t = float(np.abs(D[:3, :3]).max()), float(np.abs(D[:3, 3]).max())
                        if d_rot > 1e-6 or d_t > 1e-6:
                            off.append({"product": pkg.m.pname(row), "rotation_deg": round(float(np.degrees(np.arcsin(min(1.0, d_rot)))), 3), "translation_m": round(d_t, 6)})
                        else:
                    ...
                    boxes_k = [pkg.placed_box([item]) for item in clusters[k] if pkg.solids(item[0])]
        ...
        assets[name] = {
            ...
            "colours_srgb": sorted({c for e in nodes for c in e["colours_srgb"]}),
            "bbox_local_m": [
